...
-- manual O(n^2) version
distinct [] = True
distinct (x:xs) = distinct xs && distinct' x xs

-- distinct' x xs checks if xs does NOT contain x
distinct' _ [] = True
distinct' x (y:ys)
    | x==y = False
    | otherwise = distinct' x ys
...
incrementKey _ [] = []
incrementKey update ((key, val):xs)
    | update==key = (key, val+1) : incrementKey update xs
    | otherwise = (key, val) : incrementKey update xs
...
-- findWithDefault is too easy, let's see if we can just deal with the Maybe's directly
winner :: Map.Map String Int -> String -> String -> String
winner scores player1 player2 =
    case (score1, score2) of (_, Nothing) -> player1
                             (Nothing, _) -> player2
                             (Just a, Just b) -> if b > a then player2 else player1 
    where score1 = Map.lookup player1 scores
          score2 = Map.lookup player2 scores
...
freqs = foldr (Map.alter incrementFunc) (Map.fromList [])

incrementFunc Nothing = Just 1
incrementFunc (Just a) = Just (a+1)
...
transfer from to amount bank =
    case (fromAmount, toAmount) of (Nothing, _) -> bank
                                   (_, Nothing) -> bank
                                   (Just a, Just b) ->
                                    if amount > a || amount < 0 then bank
                                    else transfer' from to amount bank
    where fromAmount = Map.lookup from bank
...
maxIndex = fst . maximumBy (\x y -> compare (snd x) (snd y)) . Data.Array.assocs
