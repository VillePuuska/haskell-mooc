...
-- is much easier and neater, but it's O(n^2)
isOrdered :: Ord a => Tree a -> Bool
isOrdered = isOrdered' Nothing Nothing

isOrdered' :: Ord a => Maybe a -> Maybe a -> Tree a -> Bool
isOrdered' _ _ Empty = True
isOrdered' Nothing Nothing (Node val left right) =
  isOrdered' Nothing (Just val) left && isOrdered' (Just val) Nothing right
isOrdered' (Just lower_bound) Nothing (Node val left right) =
  val >= lower_bound && isOrdered' (Just lower_bound) (Just val) left && isOrdered' (Just val) Nothing right
isOrdered' Nothing (Just upper_bound) (Node val left right) =
  val <= upper_bound && isOrdered' Nothing (Just val) left && isOrdered' (Just val) (Just upper_bound) right
isOrdered' (Just lower_bound) (Just upper_bound) (Node val left right) =
  val >= lower_bound && val <= upper_bound && isOrdered' (Just lower_bound) (Just val) left && isOrdered' (Just val) (Just upper_bound) right
...
search val (Node value left right)
  | val==value = Just []
  | otherwise = case search val left of (Just ls) -> Just (StepL:ls)
                                        _ -> case search val right of (Just ls) -> Just (StepR:ls)
                                                                      _ -> Nothing
