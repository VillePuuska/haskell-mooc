...
{- Oh my god I spent way too long misunderstanding the problem.
Did not realize we should only add exactly two lines.
Well this works for arbitrarily many.

joinToLength :: Int -> [String] -> [String]
joinToLength n ls
    | n<0 = []
    | n==0 = [""]
    | otherwise = [x++y | x <- ls, y <- joinToLength (n-length x) ls]
 -}
joinToLength :: Int -> [String] -> [String]
joinToLength n ls = [x++y | x <- ls, y <- ls, length (x++y) == n]
...
interpreter' _ _ [] = []
interpreter' x y (cmd:cmds) = case cmd of "up" -> interpreter' x (y+1) cmds
                                          ...
                                          _ -> [] -- we should never be here but let's deal with it somehow anyway