...
visit maze place = do
  visited <- get
  if not (elem place visited) then do
    modify ([place]++)
    let (Just adj) = lookup place maze
    forM_ adj (visit maze)
  else return ()
...
findSum2 ks ns = do
  x <- ks
  y <- ks
  if (elem (x+y) ns) then [(x,y,x+y)] else []
...
instance Monad SL where
  ...
  return x = SL (\s -> (x,s,[]))
  (>>=) (SL f) g = SL h
    where h x = let (y,s,ls) = f x
                    (val,state,ms) = runSL (g y) s
                in (val,state,ls++ms)
...
mkCounter = do
  counter <- newIORef 0
  return (inc counter, get counter)
  where inc counter = do modifyIORef counter (+1)
        get counter = do readIORef counter
