...
parseCommand [a,b] = if a==(T.pack "balance") then Just (Balance b)
                     else Nothing
parseCommand [a,b,c] = case (readMaybe (T.unpack c) :: Maybe Int)
                       of Nothing -> Nothing
                          (Just dep) -> if a==(T.pack "deposit") then Just (Deposit b dep)
                                        else if a==(T.pack "withdraw") then Just (Withdraw b dep)
                                        else Nothing
...
perform db (Just (Deposit acc amt)) = do
  deposit db acc amt
  return $ T.pack "OK"
perform db (Just (Withdraw acc amt)) = do
  deposit db acc (-amt)
  return $ T.pack "OK"
...
simpleServer request respond = respond $ responseLBS status200 [] txt
  where txt = LB.fromStrict (encodeUtf8 (T.pack "BANK"))
