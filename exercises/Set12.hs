...
instance Functor TwoList where
  ...
  fmap f (TwoNode x y ls) = TwoNode (f x) (f y) (fmap f ls)
...
count e ls = helper e (toList ls)
  where helper _ [] = 0
        helper e (x:xs) | e==x = 1 + helper e xs
                        | otherwise = helper e xs
...
inBoth xs ys = helper (toList xs) (toList ys)
  where helper [] _ = []
        helper (x:xs) ys | elem x ys = x : helper xs ys
...
instance Foldable List where
  foldr f x Empty = x
  foldr f x (LNode y ys) = f y (foldr f x ys)
...
instance Foldable TwoList where
  ...
  foldr f x (TwoNode y z ys) = f y (f z (foldr f x ys))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node x left right) = mappend (sumTree left) (mappend x (sumTree right))
