...
totalPrice (MkShoppingEntry _ price amt) = price * fromIntegral amt
...
data Position = MkPosition {x, y :: Int}
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = [(x1+x2)/2, (y1+y2)/2, (z1+z2)/2]
                  where [x1, y1, z1] = rgb c1
                        [x2, y2, z2] = rgb c2
rgb (Invert c) = [1-x, 1-y, 1-z]
                 where [x, y, z] = rgb c
...
-- Wanted to solve with record syntax but tests say this isn't allowed.
{- data KeyVals k v = Empty | Pair {key :: k, value :: v, rest :: KeyVals k v}
  deriving Show

toList :: KeyVals k v -> [(k,v)]
toList Empty = []
toList p = (key p, value p) : toList (rest p)

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList ((key, value):xs) = Pair {key = key, value = value, rest = fromList xs} -}
...
-- wrapping a helper function with Just
{- toNat :: Int -> Maybe Nat
toNat z
  | z<0 = Nothing
  | z==0 = Just Zero
...
-- yanking Nat from Just Nat
toNat :: Int -> Maybe Nat
toNat z
  | z<0 = Nothing
  | z==0 = Just Zero
  | otherwise = Just . PlusOne $ n where Just n = toNat (z-1)
...
-- kind of ugly to need a separate function so I can handle toBin 0 = O End separately
toBin :: Int -> Bin
toBin n
  | n<=0 = O End
  | otherwise = toBin' n
...
toBin' 0 = End
toBin' n = current_bit $ toBin' (n `div` 2)
           where current_bit = if n `mod` 2 == 1 then I else O
